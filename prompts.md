...
1. 用户的生成历史中不应该包含情感标签。例如，如果用户生成的文本是"你好，我很高兴见到你。(高兴)"，那么生成历史中应该只显示"你好，我很高兴见到你。"，而不包含"(高兴)"。你需要检查前端代码中处理生成历史记录的部分，确保它在保存和显示生成历史时正确地移除了情感标签。
...
完成以下任务：
<task>
1. 目前的前端支持多种格式的音频文件上传用于音色克隆测试。请确认以下格式的音频文件均能被前端正确上传并被后端接受：WAV, MP3,。你需要在前端代码中检查文件上传组件，确保它支持这些格式，并在后端代码中检查上传处理逻辑，确保它能正确处理这些格式的文件。
2. 编写并运行自动化测试脚本，验证上述音频格式的上传功能。
3. 你需要尽可能的参考"https://docs.fish.audio/api-reference"及其中包含的相关子页面的文档来理解后端API的要求和限制，以确保上传功能符合fish audio的规范。
</task>